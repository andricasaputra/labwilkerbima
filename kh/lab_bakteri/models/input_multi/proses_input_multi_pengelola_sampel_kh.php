...
$sql = $conn->query("SELECT lab_penguji,yang_menerimalab FROM input_permohonan_kh WHERE lab_penguji IS NOT NULL AND yang_menerimalab IS NULL");
...
$nama_sampel_lab							=htmlspecialchars($conn->real_escape_string(trim($_POST['nama_sampel_lab'][$key])));
...
if (!empty($no_sampel) && !empty($yang_menerimalab)) :
	...
	$objectData->edit("UPDATE input_permohonan_kh SET nama_sampel_lab='$nama_sampel_lab', no_sampel='$no_sampel', yang_menyerahkanlab='$yang_menyerahkanlab', yang_menerimalab='$yang_menerimalab',  nip_yang_menyerahkanlab='$nip_yang_menyerahkanlab' , nip_yang_menerimalab='$nip_yang_menerimalab' WHERE id ='$id'");
