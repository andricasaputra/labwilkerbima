...
$othersql = $objectDataParasit->infoLHU();

$fetch = $othersql->fetch_object();

if ($fetch != null) {
    
    $result_no_agenda = $fetch->no_agenda;

    $result_kepala_plh2 = $fetch->kepala_plh2;
    ...
    $subdata = array();
        ...
        if(strlen($isi) == 0){
            ...
            if (strlen($selesai) == 0) {
                ...
                <button type="button" id="tombol_input_lhu_kh" class="btn btn-kusuccess btn-xs" data-toggle="modal" data-target="#modal_input_lhu_kh" data-id="'.$data2->id.'"><i class="fa fa-plus-circle fa-fw"></i> Input</button>
            ...
            $subdata[] = '
                ...
                <a href="./lab_parasit/report/print/print_surat_hasil_uji.php?id='.$data2->id.'&nama_sampel='.$data2->nama_sampel.'" target="_blank"><button type="button" class="btn btn-warning btn-xs"><i class="fa fa-print fa-fw"></i> Print</button></a>
                ';
